...
func (m *postgresDBRepo) NewReservations() ([]models.Reservation, error) {
	...
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone,r.processed, r.start_date,
		r.end_date, r.room_id, r.created_at, r.updated_at,
		rm.id,rm.room_name
		from reservations r
		left join rooms rm on (r.room_id = rm.id)
...
// GetRestrictionsRoomByDate returns restriction based on start and end date
func (m *postgresDBRepo) GetRestrictionsRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
